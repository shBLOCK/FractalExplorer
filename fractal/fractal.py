...
class Uniform:
    def __init__(self, fractal: "Fractal", name: str, typ: Type[_VarTypes]):
...
class Fractal:
    ...
    def _glslAsComplex(expr: str, tp: Type):
        ...
        if tp is float or tp is int:
            expr = Fractal._glslAsFloat(expr, tp)
    ...
    def _astToGLSL(node: ast.AST, variables: List[Tuple[str, Type[_VarTypes]] | Uniform]) -> Tuple[str, Optional[Type]]:
        match type(node):
            ...
            case ast.Name:
                ...
                typ = Fractal._getVarType(node.id, variables)
                if typ is None:
            ...
            case ast.AugAssign:
                ...
            case ast.UnaryOp:
                operand, operand_type = Fractal._astToGLSL(node.operand, variables)
                match type(node.op):
                    case ast.UAdd:
            ...
            case ast.BinOp:
                left, left_type = Fractal._astToGLSL(node.left, variables)
                right, right_type = Fractal._astToGLSL(node.right, variables)
                ...
                match type(node.op):
                    case ast.Add | ast.Sub | ast.Mult | ast.Div as op_type:
                        ...
                        if left_type is complex or right_type is complex:
                            left = Fractal._glslAsComplex(left, left_type)
                            right = Fractal._glslAsComplex(right, right_type)
                            output_type = complex
                        elif left_type is float or right_type is float:
                            left = Fractal._glslAsFloat(left, left_type)
                            right = Fractal._glslAsFloat(right, right_type)
                            output_type = float
                        match op_type:
                            case ast.Add:
            ...
            case ast.BoolOp:
                ...
                for exp_ast in node.values:
                    expr, expr_type = Fractal._astToGLSL(exp_ast, variables)
            ...
            case ast.IfExp:
                test, test_type = Fractal._astToGLSL(node.test, variables)
                if test_type is not bool:
                    raise CompilationError(f"Test expression must evaluate to a boolean, got {test_type.__name__}.", node)
                body, body_type = Fractal._astToGLSL(node.body, variables)
                els, els_type = Fractal._astToGLSL(node.orelse, variables)
            ...
            case ast.Return:
                return f"return {Fractal._astToGLSL(node.value, variables)[0]};", None
            case ast.Expr:
                warnings.warn(CompilationWarning("Result unused.", node))
                return Fractal._astToGLSL(node.value, variables)
            case ast.Pass:
                return "", None
    ...
    def resolve(self, compile_glsl=True):
        ...
        for stmt in self._ast.body:
            uni = Fractal._stripUniformFromStmt(stmt)
            if uni is not None:
        ...
        if compile_glsl:
            ...
            for stmt in statements_without_uniform:
                # noinspection PyTypeChecker
                self._glsl_func_body.append(Fractal._astToGLSL(stmt, variables)[0])
        ...
        namespace = Fractal._generateNameSpaceForExec()
